...
module Value =
  ...
  let rec toStringList d = match d with
    | [] -> []
    | h::q -> [(Card.toString h)]@toStringList q

  let rec toStringListVerbose d = match d with
  | [] -> []
  | h::q -> [(Card.toStringVerbose h)]@toStringListVerbose q

  let drawCard d = match d with
